...
import numba
from seir_individual import run_complete_simulation, compute_reported_cases
import pandas as pd
import datetime
...
parser = argparse.ArgumentParser(description='Run UP parameter sweeps in a distributed way.')
...
parser.add_argument('--sim_name', type=str, default="up_batch", help='Always specify a new directory for your jobs!')

args = parser.parse_args()

p_infect_given_contact_list = [0.024, 0.028, 0.03, 0.032, 0.034, 0.036, 0.038, 0.04]
mortality_multiplier_list = [2, 4, 6, 8, 10]
start_date_list = [2,4, 8,10,12]
...
if args.do_the_math:
    ...
    print("mkdir %s; sbatch --array=0-%s job.parameter_sweep_up.sh %s %s %s %s"%(sim_path, n_indices-1, N_SIMS_PER_COMBO, N_SIMS_PER_JOB, args.sim_name, args.seed_offset))
    print()
...
if country =="Maharashtra" or country=='up':
    params['social_dist_factor']=6.0 # TODO
else:
...
d_stay_home_start = date(2020, 3, 24)
...
def read_contact_matrix(country):
    ...
    """
    if country=='up' or country=='Maharashtra':
        country='India'
...
for i in range(N_SIMS_PER_JOB):
    ...
    those_exposed = np.logical_and(time_exposed <= 10, time_exposed > 0)
    print('those_exposed')
    print(those_exposed)

    print('num_infected_by')
    print(num_infected_by)

    print('r0_total before')
    print(r0_total)
    
    r0_total[i] = num_infected_by[np.logical_and(time_exposed <= 10, time_exposed > 0)].mean()
...
for i,D in enumerate(D_per_time):
    #print ("LOOK HERE:", f, l , time_from_d0,len(D[f:l]), len(actual_deaths), actual_deaths)
