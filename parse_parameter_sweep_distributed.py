...
if country == 'Italy':
    ...
    p_infect_given_contact_list = [0.016, 0.017, 0.018, 0.019, 0.0195, 0.02, 0.021, 0.022]
    mortality_multiplier_list = [1]
    start_date_list = [8, 10, 12, 13, 15, 17, 19]
...
if country == 'Italy':
    ...
    data = pd.read_csv('validation_data/Maharashtra/maharashtra_data_deaths.csv')
elif country== 'up':
...
if country == 'Maharashtra':
    ...
    N_COMBOS = len(list(product(p_infect_given_contact_list, mortality_multiplier_list, start_date_list)))
    NUM_DATES = len(start_date_list)
    #runs = ['maharashtra_batch_0407_correctEnd']
    runs = ['maharashtra_batch_0410']
...
def get_data(datatype, i, name_templates, num_d, num_ages=5):
    ...
    for ind,name_template in enumerate(name_templates):
        for trial_num in range(NUM_JOBS[ind]):
            ...
            if datatype=='r0' and not OLD_FORMATTING:
                datatype='r0_tot'
            ...
            if not OLD_FORMATTING:
                fname = name_template%(i, 0, pigc_val, mm_val, sd_val, datatype)
            else:
...
print()
print('Other credible intervals')
for cutoff in [0.05, 0.1, 0.2]:
    percentile_data = df_percentile_mean.to_numpy()
    include = (percentile_data <= 1 - cutoff) * (percentile_data >= cutoff)
    include = percentile_data*(1-percentile_data) >= 0.2
...
if country == 'China':
    ...
    d0 = date(2020, 3, target_start) # start
    d_lockdown = date(2020, 3, 24) # lockdown
    d_lockdown_end=date(2020, 4, 15)
    d_end = date(2020, 4, 10) # stop

t_lockdown = (d_lockdown - d0).days
t_lockdown_end=(d_lockdown_end - d0).days
plt.figure()
time_from_d0 = []
...
plt.ylim(0, 1.1*deaths_target_date[:, -1].max())
plt.xlim(0,deaths_target_date.shape[1]-1)
plt.ylabel('Total deaths', fontsize=23)
#plt.xlabel('t', fontsize=20)
if country != 'Republic of Korea':
    plt.vlines(t_lockdown, 0, 1.1*deaths_target_date[:, -1].max(), linestyles = '--', color='r')
    plt.vlines(t_lockdown_end, 0, 1.1*deaths_target_date[:, -1].max(), linestyles = '--', color='y')
else:
...
preds = np.median(deaths_target_date, axis=0)[time_from_d0[0]:]
np.corrcoef(deaths, preds)