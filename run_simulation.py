...
import numba
import sys
from seir_individual import run_complete_simulation, compute_reported_cases, get_lockdown_factor_age
...
country_list=['China','France', 'Germany', 'Iran', 'Italy',  'Spain', 'Switzerland',
              'Republic of Korea', 'UK', 'USA', 'Maharashtra', 'up']
"""list: List of modeled countries/states in India."""
...
elif country== 'Maharashtra':
    # What dates are we simulating
    d0 = date(2020, 3, 3) # start
    d_lockdown = date(2020, 3, 24) # lockdown
    d_end = date(2020, 6, 7) # stop
    d_lockdown_end =date(2030, 4, 15)#end lockdown
    d_stay_home_start = date(3000, 3, 8)
    d_t1=date(2020, 4, 15)    # Policy decision time step
    d_t2=date(2020, 4, 21)    # Policy decision time step
    d_t3=date(2020, 4, 30)    # Policy decision time step
    d_t4=date(2020, 5, 7)     # Policy decision time step
elif country== 'up':
    # What dates are we simulating
    d0 = date(2020, 3, 10) # start
    d_lockdown = date(2020, 3, 24) # lockdown
    d_end = date(2020, 6, 7) # stop
    d_lockdown_end =date(2030, 4, 15)#end lockdown
    d_stay_home_start = date(3000, 3, 8)
...
if country == 'China':
    ...
    params['initial_infected_fraction'] = 5./params['n']
elif country == 'Maharashtra' or country=='up':
    params['initial_infected_fraction'] = 5./params['n'] 
 
    
params['t_lockdown'] = float((d_lockdown - d0).days)
params['t_lockdown_end']=float((d_lockdown_end - d0).days)
...
""" Set lockdown policy parameters (severity levels, i.e. lockdown factors) below"""

if country =="Maharashtra" or country=='up':
    params['social_dist_factor']=2.0 
    params['mild_dist_factor']=6.0  
else:
    params['social_dist_factor']= 10.
    params['mild_dist_factor']=10.0

lockdown_factor_dict={'L':params['lockdown_factor'], 'M':params['mild_dist_factor'], 'S':params['social_dist_factor']}

save_subfolder=country+'_params_p0.036_dmult2_dstart3.10'    # Part of directory path where policy simulation results will be saved
LOCKDOWN_POLICY_ARRAY=['LLLL','SSSS', 'LSSS', 'LLSS', 'LLLS','SLLS','LSLS','SLSL','LLSL','MMMM','MSSS', 'MMSS', 'MMMS', 'LLMM', 'LLMS', 'LLLM', 'LMLM' ,'LMMS']
LOCKDOWN_POLICY=LOCKDOWN_POLICY_ARRAY[0]                       # For now, run the first lockdown policy

#LOCKDOWN_POLICY=LOCKDOWN_POLICY_ARRAY[int(sys.argv[1])//50]   # use this for running on a computing cluster with 50 seeds
#seed = int(sys.argv[1])%50                                    # Uncomment this to run simulations over 50 different seeds on a cluster
### With 50 seeds per policy, remember to run (# of jobs) = 50* len(LOCKDOWN_POLICY_ARRAY)
"""Set the time points where the policy interventions occur and set the lockdown severity for each policy that is brought into effect"""
params['t1']=float((d_t1 - d0).days)
params['t2']=float((d_t2 - d0).days)
params['t3']=float((d_t3 - d0).days)
params['t4']=float((d_t4 - d0).days)
params['factor_t1']=lockdown_factor_dict[LOCKDOWN_POLICY[0]]
params['factor_t2']=lockdown_factor_dict[LOCKDOWN_POLICY[1]]
params['factor_t3']=lockdown_factor_dict[LOCKDOWN_POLICY[2]]
params['factor_t4']=lockdown_factor_dict[LOCKDOWN_POLICY[3]]
...
if country == 'China' or country == 'Republic of Korea':
    ...
    params['p_infect_given_contact'] = 0.032 
elif country == 'up':
    params['p_infect_given_contact'] = 0.036   
...
if country == 'Italy':
    ...
    params['mortality_multiplier'] = 8.0 
elif country == 'up':
    params['mortality_multiplier'] = 6.0 
else:
...
isolation_filename='./isolation_exps/default_for_all.pickle'
...
##### 2.6/4.6=0.565;   1.6/4.6=0.347;  0.6/4.6=0.130;     5.6/4.6=1.217;    
...
def read_contact_matrix(country):
    ...
    """
    if country=='up' or country=='Maharashtra':  # For these indian states, read the contact data for India
        country='India'            
...
for i in range(num_runs):
    ...
    num_infected_asympt, age, time_infected, time_to_severe \
        =  run_complete_simulation(seed,country, contact_matrix, p_mild_severe, p_severe_critical, \
...
#    pickle.dump([S.sum(axis=1), E.sum(axis=1), Mild.sum(axis=1), Severe.sum(axis=1), Critical.sum(axis=1), R.sum(axis=1), D.sum(axis=1)], open(isolation_filename, "wb" ))
#    
#    print("CFR:"+str(D_m[test_t]/(D_m[test_t]+Mild_m[test_t]+Critical_m[test_t]+Severe_m[test_t])))
#    print("D/(R+D):"+str(D_m[test_t]/(D_m[test_t]+R_m[test_t])))
    print ("RO for this seed: ",num_infected_by[np.logical_and(time_exposed <= 10, time_exposed > 0)].mean())
    print ("Deaths, documented, infected:", D[-1].sum(), Documented[-1].sum(), Severe[-1].sum()+Mild[-1].sum()+Critical[-1].sum())
    print(d0, params['p_infect_given_contact'], D.sum(axis=1)[-1], Documented.sum(axis=1)[-1], total_infections_time[i, -1])
#import pickle
#pickle.dump((r_0_over_time, cfr_over_time, fraction_over_70_time, fraction_below_30_time, median_age_time, total_infections_time, total_documented_time, dead_by_age, total_deaths_time, all_final_s), open('results_calibration_{}_{}_{}.pickle'.format(country, params['p_infect_given_contact'], d0.day), 'wb'))
    if not os.path.isdir('policy_results/'+save_subfolder):
        if os.path.isdir('policy_results'):
            os.mkdir('policy_results/'+save_subfolder)
        else:
            os.mkdir('policy_results')
            os.mkdir('policy_results/'+save_subfolder)
    np.save('policy_results/'+save_subfolder+'/deaths_policy'+LOCKDOWN_POLICY+'_s%s'%(seed),total_deaths_time)
    np.save('policy_results/'+save_subfolder+'/infections_policy'+LOCKDOWN_POLICY+'_s%s'%(seed),total_infections_time)
    np.save('policy_results/'+save_subfolder+'/r0_policy'+LOCKDOWN_POLICY+'_s%s'%(seed),r_0_over_time)
#np.save('policy_results/others_policy1001_s%s'%(seed), np.array([r_0_over_time,cfr_over_time,fraction_over_70_time,fraction_below_30_time,median_age_time,total_documented_time,dead_by_age]))
